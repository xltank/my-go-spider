...
func main() {
	...
	c := colly.NewCollector(
		...
		//colly.AllowedDomains("news.sina.com.cn"),
	...
	c.OnHTML(".ConsTi a", func(e *colly.HTMLElement) {
		link := e.Attr("href")
		// Print link
	...
	err := c.Visit("https://news.sina.com.cn/hotnews/")
	if err != nil {
...
func UpsertOneByURI(col string, uri string, data interface{}) {
	...
	r, err := db.DB.Collection(col).UpdateOne(
		...
		bson.M{
			"$set":         d,
			"$setOnInsert": bson.D{{"createdAt", time.Now()}},
		},
